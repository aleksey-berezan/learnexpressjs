...
appServer.setDb(testDb);

// init prod db
const prodDb = new sqlite3.Database('./database.sqlite');
console.log(`Seeding db: ${JSON.stringify(prodDb)}`);
const initializeDb = require('../migration.js');
initializeDb(prodDb);
  ...
  it('should return all currently-employed artists', function () {
    return request(app)
      ...
      .then(function (response) {
        ...
        expect(artists.find(artist => artist.id === 3)).to.exist;
        expect(artists.find(artist => artist.id === 4)).to.not.exist;
      });
  });
